    ...
    <Card className="mb-4 overflow-hidden hover:shadow-md transition-shadow">
      <CardContent className="p-4">
        <div className="flex items-center mb-3">
          <Link to={`/user/${post.author.id}`} className="flex items-center">
            <Avatar className="h-6 w-6 mr-2">
              ...
              <AvatarFallback className="bg-forum-100 text-forum-700 text-xs">
        ...
        <Link to={`/post/${post.id}`}>
          <h2 className="text-lg font-medium mb-2 hover:text-forum-700 transition-colors">
            {post.title}
          </h2>
      ...
      <CardFooter className="px-4 py-2 bg-gray-50 flex justify-between">
        <div className="flex items-center space-x-4">
          <Button 
            variant="ghost" 
            size="sm" 
            className={`text-xs flex items-center ${liked ? 'text-forum-600' : 'text-gray-600'}`}
            onClick={handleLike}
          >
